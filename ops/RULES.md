...
### Gemini連携（Discordセッション限定）

**重要：** ClaudeがDiscord経由で動いている場合、GeminiはDiscordメッセージのタグで呼び出す。

**呼び方：返答テキストに `【Geminiへ】` タグを含めるだけ。**
Harugemini botが自動でメッセージを受信し、Gemini CLIを実行して返答する。

```
例: 【Geminiへ】HomeScreen.tsx のブリーフィングカードを会話的なデザインに改善して。現在のコード: [コード]
```

| タグ | 用途 |
|------|------|
| `【Geminiへ】` | Geminiに実装・相談を依頼 |
| `【レビュー依頼】` | コード・計画のレビューを依頼 |

**Geminiの役割：** UI/UXデザイン・フロントエンド実装担当
**Claudeの役割：** バックエンド・アーキテクチャ・調整担当

⚠️ `gemini` コマンドをBashで直接実行してはいけない。タグ方式のみ使うこと。
