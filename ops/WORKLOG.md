...
### 2026-03-18
- Owner: Codex
- Scope: G3 食事解析の再利用ヒット改善と微量栄養素補完
- Result: 店名と商品名を一緒に入れた食事名でも、過去に保存した候補へ当たりやすく改善。あわせて、公式サイトなどの検索結果で細かな栄養が多く欠ける場合は、不足分だけ補って保存し、次回からより揃った内容をそのまま再利用できるようにした。型整合性確認は成功。
- Files: 食事解析処理、進捗ボード、作業ログ、引き継ぎメモ
- Risk/Follow-up: 実運用ではチェーン店メニュー名を店名込みで2回試し、2回目に再利用されることと、細かな栄養が初回より揃っていることを確認すると安全。

- Owner: Codex
- Scope: G2 食事解析の公式値検索と自動再利用
- Result: 食事名だけを入れた解析で、まず保存済み候補を探し、見つからない場合は公式サイトなどを検索して栄養値を返せるようにした。最初の検索結果は次回から自動で再利用されるため、同じ食事では待ち時間と外部利用コストを抑えやすくした。写真つき入力は従来どおり画像解析を維持。型整合性確認は成功。
- Files: 食事解析処理、進捗ボード、作業ログ、引き継ぎメモ
- Risk/Follow-up: 実運用ではチェーン店メニュー名を同じ文面で2回試し、初回は検索結果、2回目は再利用結果になることを確認すると安全。

- Owner: Codex
- Scope: G1 食事写真解析のOpenAI切替対応
- Result: 食事解析で、保存済み候補が見つからない場合に OpenAI も使えるようにし、食事写真の解析先を環境設定で切り替えられる状態にした。微量栄養素も食材と調理法から推定する指示を強め、画像解析時に細かな栄養が抜けにくくした。型整合性確認は成功。
- Files: 食事解析処理、環境設定の型、進捗ボード、作業ログ、引き継ぎメモ
- Risk/Follow-up: 実運用では食事写真つきの1件で OpenAI 側の応答内容を目視確認すると安全。必要なら食事解析だけ別モデル指定も可能。

- Owner: Codex
- Scope: レポート詳細画面の見分け改善と履歴導線追加
- Result: 週次・月次・追加レポートの詳細画面で、見出しに種類の目印が付き、ホームへ戻ったときにどこから来たか分かりやすくした。あわせて本文の下から同じ種類の履歴一覧へ戻れる導線を追加。型確認は成功。
- Files: レポート詳細画面、画面遷移管理、進捗ボード、作業ログ、引き継ぎメモ
- Risk/Follow-up: 実機またはブラウザで週次・月次・追加レポートの3パターンを一度ずつ触り、見出しの目印と履歴遷移が意図どおりか目視確認すると安全。
...
- Result: 健康スコアを「睡眠・活動・栄養・コンディション」の4領域に再編し、活動に移動距離、栄養に摂取バランス、コンディションに血圧・安静時心拍・体組成の統合評価を追加。ホームの状態表示も同じ4領域に統一し、動作確認まで完了。
- Files: スコア計算・気づき生成・ホーム表示の実装、画面側の受け取り定義、依頼ステータス更新
- Risk/Follow-up: 既存クライアントの表示項目が旧キー前提の場合は、同時反映されているか最終確認が必要
...
### 2026-03-04
- Owner: Codex
- Scope: I2v3 健康タブのルールベースアドバイス化
- Result: 健康タブ下部の共通コメントを廃止し、体重・血圧/心拍・睡眠の各タブ上部に、測定値に応じて変わるアドバイス表示を追加。測定値が不足する日はアドバイスを出さない動きに統一。ダッシュボード上で I2v3 を完了に更新。
- Files: 健康タブ画面、健康タブスタイル、CEOダッシュボード、ハンドオフ
- Risk/Follow-up: 文言トーンは実データ確認後に微調整余地あり。

### 2026-03-04 (2)
- Owner: Codex
- Scope: F1 睡眠ステージの二重カウント修正
