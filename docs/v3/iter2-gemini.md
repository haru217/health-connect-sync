...
## ⚠️ Iteration 1 の修正（先に対応してください）

`HomeScreen.tsx` と `HomeScreen.css` の間にクラス名の不一致があります。

**修正 1: `HomeScreen.tsx` の `SufficiencyBar`**

```tsx
// 修正前
className={`sufficiency-dot ${sufficiency[item.key] ? 'on' : 'off'}`}
// 修正後
className={`sufficiency-pill ${sufficiency[item.key] ? 'on' : 'off'}`}
```

**修正 2: `HomeScreen.tsx` の `EvidenceList`**
...
--text-primary: #4A4A4A      /* 主テキスト */
--text-muted: #8FA39A        /* 補助テキスト */
